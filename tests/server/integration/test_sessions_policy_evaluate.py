...
async def test_evaluate_actor_prefers_user_id_over_session_owner(
    client: httpx.AsyncClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """
    When the HTTP caller is authenticated, ``get_session_owner`` must not
    be called — the authenticated user_id wins without a DB round-trip.
    """
    policy = FunctionPolicySpec(
        name="admin__deny_blocked_actor",
        on=None,
    ...
    def _spy_get_session_owner(_self: object, _conversation_id: str) -> str | None:
    ...
    monkeypatch.setattr(SqlAlchemyConversationStore, "get_session_owner", _spy_get_session_owner)
    ...
    assert not owner_called, "get_session_owner should not be called when user_id is set"


async def test_evaluate_actor_falls_back_to_session_owner_when_unauthenticated(
    client: httpx.AsyncClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """
    When the HTTP caller has no auth (unauthenticated runner), the session
    owner is used as the actor so identity-based policies still fire.
    """
    policy = FunctionPolicySpec(
        name="admin__deny_blocked_actor",
        on=None,
    ...
    # Simulate unauthenticated runner (no user_id on the HTTP request).
    monkeypatch.setattr(
        "omnigent.server.routes.sessions._get_user_id",
    ...
    monkeypatch.setattr(
        SqlAlchemyConversationStore,
        "get_session_owner",
        lambda _self, _conv_id: "blocked@test.com",
    )
    ...
    body = resp.json()
    assert body["result"] == "POLICY_ACTION_DENY", (
        "session owner identity should be used when caller is unauthenticated"
    )
    assert body["reason"] == "Blocked user"
