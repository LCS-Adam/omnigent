...
Like goose-native, this bridge writes no vendor config: Hermes' model/provider,
tools, and skills come from the user's own ``hermes setup`` / ``hermes model``
config under ``~/.hermes`` (``$HERMES_HOME``), which Omnigent does not mutate for
the native TUI. (The per-session ``HERMES_HOME`` with Omnigent policy hooks is the
*headless* ``hermes`` harness's concern — see :mod:`omnigent.inner.hermes_executor`
— not this one; the TUI's own ``pre_tool_call`` approval prompts are the tool gate.)
"""
