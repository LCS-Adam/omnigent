...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Report stale bindings without clearing them.",
)
@click.option(
    "--yes",
    "-y",
...
def session_repair(server: str | None, dry_run: bool, yes: bool) -> None:
    """Find and clear stale runner bindings from stranded sessions.

    Iterates all visible sessions, checks whether each bound runner is
    online, and clears stale bindings via PATCH /v1/sessions/{id}.
    Sessions with no runner binding are skipped.  Clearing a stale
    binding makes the session resumable — the next ``omnigent run
    --resume`` or ``omnigent claude --resume`` spawns a fresh runner.

    \b
    Examples:
      omnigent session repair --dry-run
      omnigent session repair --yes
      omnigent session repair --server https://myserver.com --yes
    """
    import httpx
    ...
    with httpx.Client(
        ...
        bound = [s for s in sessions if s.get("runner_id")]
        if not bound:
            click.echo("No sessions with runner bindings found.")
            return

        click.echo(
            f"Checking liveness of {len(bound)} runner binding(s) across "
        ...
        for s in bound:
            ...
            r = client.get(f"/v1/runners/{runner_id}/status")
            online = r.status_code == 200 and bool(r.json().get("online"))
            if not online:
                stale.append(s)

        if not stale:
            click.echo("All bound runners are online. Nothing to repair.")
            return

        click.echo(f"\nFound {len(stale)} stale binding(s):")
        for s in stale:
            click.echo(f"  {s['id']}  →  runner {s['runner_id']}")
        ...
        if not yes:
            click.confirm(f"\nClear {len(stale)} stale binding(s)?", abort=True)

        cleared = 0
        failed = 0
        for s in stale:
            r = client.patch(f"/v1/sessions/{s['id']}", json={"runner_id": ""})
            if r.status_code < 400:
                cleared += 1
                click.echo(f"  cleared {s['id']}")
            else:
                failed += 1
                click.echo(f"  FAILED {s['id']}: HTTP {r.status_code} {r.text[:120]}")
