...
- **Phase 1 — First-class entity:** `projects` table + CRUD, empty projects
  ...
  **optional** and, if added, client-only (§7.2). No label backfill here — the
  ...
  label-projects without migrating them.
...
### Done (Stage 1 — first-class entity foundation)
- ✅ **`projects` table** — `SqlProject` (`db_models.py`): `id` (Uuid16),
  `name`, `owner_user_id`, `created_at`, `updated_at`. Owner-scoped indexes;
  per-owner name uniqueness enforced in the store. (No `config` column yet —
  deferred to Phase 2 as a small add-column migration alongside the code that
  reads it, so we don't ship an unused column.)
- ✅ **Membership link** — nullable `project_id` (Uuid16) column on
  `SqlConversationMetadata`; no DB FK (Rule R032). `NULL` = unfiled.
- ✅ **Migration** `b1c2d3e4f5a6` — creates the table + adds the column;
  additive, nullable, no backfill. Chained after `d7f1a2b3c4e5`.
- ✅ **Entity** — `Project` (`entities/project.py`); `Conversation.project_id`
  populated in `_to_conversation`.
- ✅ **Store** — `ProjectStore` + `SqlAlchemyProjectStore` (create/get/list/
  update/delete, owner-scoped).
- ✅ **Membership ops on the conversation store** —
  `set_conversation_project()` (set / move / unfile) and
  `list_conversations(project_id=...)` filter (`""` = unfiled), cross-DB-safe.
- ✅ **API** — `POST/GET/PATCH/DELETE /v1/projects` (`routes/projects.py`),
  request/response schemas, wired into `create_app` + CLI; `openapi.json`
  regenerated.
- ✅ Tests: store CRUD + owner scoping + name uniqueness; route CRUD;
  conversation-store membership filing/filtering.

### TODO
- ⬜ **Wire `set_conversation_project` into the sessions PATCH route** — the
  store method exists and is tested, but the "move session into/out of a
  project" HTTP action is not yet exposed (§6). This is the immediate next step.
- ⬜ **Expose the `project_id` filter on `GET /v1/sessions`** — the store filter
  `list_conversations(project_id=…)` is done + tested, but not yet threaded onto
  the sessions route's query params. Needed for "list sessions in project X"
  (§4.3, §6).
- ⬜ **Unify the two filters into a genuine dual-read (label OR `project_id`).**
  `list_conversations` today has *two independent, coexisting* project filters —
  `project` (v1 `omni_project` label, by name) and `project_id` (v2 metadata
  column, by id) — each consulting only its own storage. This is dual-*capable*,
  not dual-*reading*: a `project_id="X"` query does **not** also return sessions
  grouped under the equivalent label, and the two disagree on what counts as
  "unfiled". That's fine while v1 and v2 clients are separate, but **once we wire
  the new APIs into clients** (so a single client mixes both, or the backfill
  runs mid-flight), we must unify them so "a session is in project X if it has
  *either* a `project_id` *or* the `omni_project` label" (the §13 guarantee) is
  literally true. Requires the id↔name mapping, so it lands with the client
  wiring / backfill — not before. This is a real correctness gap, not cleanup.
